...
export async function POST(request: Request, { params }: { params: Promise<{ trainingId: string }> }) {
  ...
  try {
    return NextResponse.json(await adminService.bookTrainingForCustomer(trainingId, auth.user.id));
  } catch (error) {
    const message = error instanceof Error ? error.message : "Booking failed.";
    return NextResponse.json({ error: { code: "booking_failed", message } }, { status: 400 });
  }
}
